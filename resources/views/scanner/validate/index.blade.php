...
<div x-data="scannerApp()" class="space-y-6">
    ...
    <div class="flex flex-col space-y-2">
        <h2 class="text-xl font-bold">Validasi Akses Masuk</h2>
        <p class="text-xs text-gray-500">Scan QR Code Wristband untuk memvalidasi akses masuk pengunjung.</p>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="glass-dark rounded-2xl p-4 flex flex-col items-center justify-center text-center">
            <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Wristband Terbit</span>
            <span class="text-2xl font-bold text-white">{{ $stats['total_exchanged'] }}</span>
        </div>
        <div class="glass-dark rounded-2xl p-4 flex flex-col items-center justify-center text-center border-l-4 border-emerald-500">
            <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Sudah Masuk</span>
            <span class="text-2xl font-bold text-white">{{ $stats['total_validated'] }}</span>
            <span class="text-[10px] text-emerald-400 font-bold mt-1">{{ $stats['validation_percentage'] }}%</span>
        </div>
    </div>
    @endif
        ...
        <div class="space-y-4">
            ...
            <div class="glass-dark rounded-3xl overflow-hidden relative">
                <div x-show="!scannerActive" class="p-8 text-center bg-gray-900/50">
                    ...
                    <button @click="startScanner()" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 px-8 rounded-xl transition shadow-lg shadow-emerald-600/20">
                        Buka Kamera
                    </button>
                    <p class="text-[10px] text-gray-500 mt-4 uppercase tracking-widest">Siap memvalidasi wristband</p>
                </div>
            ...
            <div class="glass-dark rounded-2xl p-4 flex gap-2">
                <input type="text" 
                       ...
                       placeholder="Scan/Input manual kode wristband..."
                       class="flex-1 bg-gray-900 border border-gray-700 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500 transition">
    ...
    <div x-show="showResult" 
        ...
        <div class="w-full max-w-sm glass-dark rounded-[2.5rem] p-8 text-center relative overflow-hidden">
            <!-- Background Glow -->
            <div :class="resultSuccess ? 'bg-emerald-500/20' : 'bg-red-500/20'" class="absolute inset-0 z-0"></div>

            <div class="relative z-10">
                <!-- Icon -->
                <div :class="resultSuccess ? 'bg-emerald-500' : 'bg-red-500'" class="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl">
                ...
                <h3 x-text="resultTitle" class="text-2xl font-black mb-2 text-white italic tracking-tight uppercase"></h3>
                <p x-text="resultMessage" class="text-gray-400 text-sm mb-6"></p>

                <!-- Wristband details -->
                <template x-if="wristband">
                    <div class="bg-gray-900/50 rounded-2xl p-4 mb-6 text-left space-y-2 border border-white/5">
                        <div class="flex justify-between items-center pb-2 border-b border-gray-800">
                            ...
                            <span x-text="wristband.ticket?.ticket_category?.name" class="text-xs font-bold text-emerald-400"></span>
                        </div>
                        <div class="flex justify-between items-center" x-show="wristband.ticket?.seat">
                            <span class="text-[10px] font-bold text-gray-500 uppercase">No. Kursi</span>
                            <span x-text="wristband.ticket?.seat?.full_seat" class="text-sm font-black text-white"></span>
                        </div>
                        <div class="pt-1">
                ...
                <div class="space-y-3">
                    <template x-if="resultSuccess && canEnter">
                        <button @click="confirmEntry()" class="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-black py-4 rounded-2xl shadow-lg transition active:scale-95 tracking-wide">
                            KONFIRMASI MASUK
                        </button>
                    </template>
        ...
        return {
            ...
            onEventChange() {
                if (this.selectedEventId) {
                    window.location.href = `{{ route('scanner.validate') }}?event_id=${this.selectedEventId}`;
                }
            },
            ...
            processQR(qrCode) {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        this.canEnter = data.can_enter;
                        
                        if (this.canEnter) {
                            this.showScanResult(true, 'AKSES DIBERIKAN', 'Wristband valid. Silakan izinkan masuk.');
                            this.playSound('success');
                            this.vibrate([100, 50, 100]);
                        } else {
                            this.showScanResult(false, 'AKSES DITOLAK', 'Wristband sudah pernah digunakan atau tidak aktif.');
                            this.playSound('error');
                            this.vibrate(300);
                        }
                    } else {
                        this.showScanResult(false, 'WRISTBAND TIDAK VALID', data.message);
                        this.playSound('error');
                        this.vibrate(300);
                    }
                })
            ...
            confirmEntry() {
                if (!this.wristband || this.isProcessing) return;
                ...
                fetch('{{ route("scanner.validate.confirm") }}', {
                    ...
                    body: JSON.stringify({
                        wristband_id: this.wristband.id,
                        event_id: this.selectedEventId
                    })
                })
                .then(res => res.json())
                .then(data => {
                    this.isProcessing = false;
                    if (data.success) {
                        this.showScanResult(true, 'CONGRATS!', 'Check-in berhasil dikonfirmasi.');
                        this.playSound('success');
                        // Set canEnter to false so button disappears
                        this.canEnter = false;
                    } else {
                        this.showScanResult(false, 'GAGAL KONFIRMASI', data.message);
                        this.playSound('error');
                    }
                })
                .catch(err => {
            ...
            resetResult() {
                const wasSuccess = this.resultSuccess && !this.canEnter;
